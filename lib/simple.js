  ...
  const sock = Object.defineProperties(conn, {
    ...
    sendFile: {
      ...
      async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
        ...
        // ANTI-BAN: Simula presenza appropriata prima di inviare
        try {
            if (mtype === 'audio') {
                await conn.sendPresenceUpdate('recording', jid);
                await new Promise(resolve => setTimeout(resolve, 1500)); // minimo 1.5s per recording
            } else {
                const textLength = (caption || '').length;
                const composeTime = Math.min(Math.max(textLength * 10, 500), 1500);
                await conn.sendPresenceUpdate('composing', jid);
                if (composeTime > 0) await new Promise(resolve => setTimeout(resolve, composeTime));
            }
        } catch (e) {
            // Ignora errori di presenza
        }
...
END:VCARD
      ...
      enumerable: true,
        ...
        reply: {
            /**
             * Rispondi a un messaggio con simulazione umana anti-ban
             * @param {String} jid
            ...
            async value(jid, text = '', quoted, options) {
                // ANTI-BAN: Simula "sta scrivendo..." prima di rispondere
                try {
                    const textLength = typeof text === 'string' ? text.length : 50;
                    const composeTime = Math.min(Math.max(textLength * 10, 500), 1500); // 500ms-1500ms
                    await conn.sendPresenceUpdate('composing', jid);
                    await new Promise(resolve => setTimeout(resolve, composeTime));
                } catch (e) {
                    // Ignora errori di presenza (non critici)
                }
